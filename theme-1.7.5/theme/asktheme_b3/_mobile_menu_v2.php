            ...
            <ul class="main-menu home-nav">
                ...
                for ($i = 0; $row = sql_fetch_array($result); $i++) {
                    ...
                    <li class="nav-parent nav-item" id="headingOne<?php echo $i ?>">
                        <a href="<?php echo $row['me_link']; ?>" target="_<?php echo $row['me_target']; ?>" data-target="<?php echo "collapseOne{$i}";?>" class="nav-link parent_menu has-links">
                            <span><?php echo $row['me_name'] ?></span>
                        </a>
                        <!-- 확장 버튼 -->
                        <?php
                        $sql_count = " select count(*) as cnt from {$g5['menu_table']} where {$menu_query} and length(me_code) = '4' and substring(me_code, 1, 2) = '{$row['me_code']}' order by me_order, me_id ";
